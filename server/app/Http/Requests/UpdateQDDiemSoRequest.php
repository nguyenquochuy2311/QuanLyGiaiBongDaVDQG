<?php

namespace App\Http\Requests;

use Illuminate\Foundation\Http\FormRequest;
use Illuminate\Http\Exceptions\HttpResponseException;
use Illuminate\Contracts\Validation\Validator;

class UpdateQDDiemSoRequest extends FormRequest
{
    /**
     * Determine if the user is authorized to make this request.
     *
     * @return bool
     */
    public function authorize()
    {
        return true;
    }

    /**
     * Get the validation rules that apply to the request.
     *
     * @return array
     */
    public function rules()
    {
        return [
            'idMG' => 'required|interger',
            'DiemThang' => 'required|interger',
            'DiemHoa' => 'required|integer',
            'DiemThua' => 'required|integer'
        ];
    }

    public function failedValidation(Validator $validator)
    {
        throw new HttpResponseException(response()->json([
            'status' => false,
            'message' => 'Lỗi cập nhật dữ liệu',
            'detail' => $validator->errors()->first()
        ]));
    }

    public function messages()
    {
        return [
            'idMG.required' => 'Mùa giải không được bỏ trống!',
            'idMG.integer' => 'Mùa giải không hợp lệ!',
            
            'DiemThang.required' => 'Điểm quy định trận thắng không được bỏ trống!',
            'DiemThang.integer' => 'Điểm quy định trận thắng không hợp lệ',

            'DiemHoa.required' => 'Điểm quy định trận hoà không được bỏ trống!',
            'DiemHoa.integer' => 'Điểm quy định trận hoà không hợp lệ',

            'DiemThua.required' => 'Điểm quy định trận thua không được bỏ trống!',
            'DiemThua.integer' => 'Điểm quy định trận thua không hợp lệ',
        ];
    }
}