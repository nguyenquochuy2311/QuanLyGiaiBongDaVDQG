...
function EditMatch(props) {
  ...
  const [, setIdCLB] = useState("");
  const [, setIdCLB2] = useState("");
  const [, setThoiGian] = useState("");


  const [, setIdToTT] = useState("");
  const [, setVongDau] = useState("");
        ...
        <Col md={{ span: 4, offset: 4 }}>
          <Form className="form">
            ...
            >
              ...
              {clb.map((item) => (
                <option key={item.idCLB} value={item.idCLB}>
                  {item.TenCLB}
                </option>
              ))}
            </Form.Select>
            ...
            <Form.Group className="mb-3" controlId="formBasicEmail">
              ...
              <Form.Control
                ...
                type="date"
                // disabled={true}
                onChange={(e) => setThoiGian(formatDate(e.target.value))}
              />
              <Form.Text className="text-muted"></Form.Text>
              <Form.Label className="err">--</Form.Label>
            ...
            {/* <Link
            > */}
            <Button onClick={() => editMatch(id)} variant="outline-success">
              <b>Sửa ngay</b>
            </Button>{" "}
            {/* </Link> */}
            <br />
