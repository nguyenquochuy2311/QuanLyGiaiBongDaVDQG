...
function EditClub(props) {
  ...
  const [viet_tat, setviet_tat] = useState(data.viet_tat);
  const [draw, setDraw] = useState(data.draw);
  const [san, setsan] = useState(data.san);
...
//   && console.log('>>>>>>>>>>>..obj'+ e))
  ...
  async function showID(id) {
    let result = await fetch("http://localhost:8000/api/bxh_clb" + id);
    result = await result.json();
    Setclb(result);
    console.log(">>>>>>>>>>>>>>: " + result);
  }
...
function to_slug(str)
{
  ...
  async function editClub(id) {
    ...
    const viet_tat = document.getElementById("viet_tat");
  ...
  //get ten CLB
  const tenClB = clb.find(e=> e.idCLB === Number(id)? e :'')
  console.log(tenClB + '<<<<<<<<<<<<<<<<');
  return (
