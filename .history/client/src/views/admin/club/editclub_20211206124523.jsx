...
function EditClub(props) {
  ...
  const [VietTat, setVietTat] = useState(data.TenCLB);
  const [SanNha, setSanNha] = useState(data.TenCLB);
  const [TruSo, setTruSo] = useState(data.TenCLB);
  const [Logo, setLogo] = useState(data.TenCLB);
...
function to_slug(str)
{
  ...
  //get ten CLB
  const tenClB = data.find(e=> e.idCLB === Number(id)? e :'')
  console.log(tenClB + '<<<<<<<<<<<<<<<<');
  return (
